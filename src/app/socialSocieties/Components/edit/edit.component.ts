...
export class EditComponent implements OnInit {
  ...
  loadSociety(): void {
    this.societyService.getById(this.data.id).subscribe({
      next: (d: socialSocietiesRead) => {
        this.society = d;
        this.patchData(d);
        console.log('Society API response:', d);
  ...
  /* ================= PATCH OLD DATA ================= */
  patchData(d: socialSocietiesRead): void {
  ...
  submit(): void {
    ...
    this.societyService
      .updateSocialSocieties(this.society.id, formData)
      .subscribe({
        next: () => {
          this.loading = true;
